...
def _calculate_search_direction(grad, inv_hessian, norm_constraint):
  """
    ...
    """
  step = calculate_hvp_by_2loop_recursion(
    inv_hessian, grad
  )  # apply inverse Hessiang: g = H^-1g
...
def calculate_metrics_bfgs(opt_state):
  ...
  sy = 1.0 / rho
  return dict(
    sy_min=float(jnp.min(sy)),
    sy_max=float(jnp.max(sy)),
    sy_mean=float(jnp.mean(sy))
  )
...
def build_bfgs_optimizer(
  log_psi_squared_func, grad_loss_func, mcmc: MetropolisHastingsMonteCarlo,
  initial_params, opt_config: OptimizationConfig, n_walkers
):
  ...
  def grad_loss_flat(r, R, Z, params_flat, fixed_params, clipping_state):
    g, l = grad_loss_func(
      r, R, Z, unravel_pytree(params_flat), fixed_params, clipping_state
    )
    return ravel_pytree(g)[0], l

  bfgs_config: BFGSOptimizerConfig = opt_config.optimizer
  n_batches = n_walkers // opt_config.batch_size
  opt_init, opt_update, get_params = get_builtin_optimizer(
    bfgs_config.internal_optimizer, opt_config.schedule,
    opt_config.learning_rate
  )

  @jax.jit
  def optimize_epoch_bfgs(
    epoch, mcmc_state: MCMCState, opt_state, clipping_state, fixed_params
  ):
    ...
    def _batch_step(carry, r_batch):
      ...
      raw_grad, E_loc = grad_loss_flat(
        r_batch, mcmc_state.R, mcmc_state.Z, params_before_step, fixed_params,
        clipping_state
      )
      g = _calculate_search_direction(
        raw_grad, inv_hessian, bfgs_config.norm_constraint
      )
    ...
    (params_new, inner_opt_state), (E_epoch, gradients) = jax.lax.scan(
      _batch_step, (jnp.zeros([n_params]), inner_opt_state), r_batches
    )
    ...
    def _update_hessian(inv_hessian):
      ...
      s_new, y_new = _regularize_curvature_pair(
        s_new, y_new, bfgs_config.hessian_regularization
      )
      return update_hessian_representation(inv_hessian, s_new, y_new)

    inv_hessian = jax.lax.cond(
      epoch % bfgs_config.update_hessian_every_n_epochs == 0, _update_hessian,
      lambda x: x, inv_hessian
    )

    # Update clipping and wavefunction values
    clipping_state = calculate_clipping_state(E_epoch, opt_config.clipping)
    mcmc_state.log_psi_sqr = log_psi_squared_func(
      *mcmc_state.model_args, unravel_pytree(get_params(inner_opt_state)),
      fixed_params
    )
