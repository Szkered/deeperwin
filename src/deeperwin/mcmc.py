...
class MCMCState:
  ...
  def initialize_around_nuclei(
    cls, n_walkers, physical_config: PhysicalConfig, rng
  ):
    # TODO: add element specific density electron density as initial guess
    rng1, rng2 = jax.random.split(rng)
    r0 = np.array(
      jax.random.normal(rng1, [n_walkers, physical_config.n_electrons, 3])
    )
  ...
  @classmethod
  def resize_or_init(
    cls, mcmc_state, n_walkers, physical_config: PhysicalConfig, rng
  ):
    if mcmc_state:
  ...
  def split_across_devices(self):
    ...
    assert len(self.r) % jax.device_count(
    ) == 0, f"Number of samples ({len(self.r)}) is not evenly divisible across devices ({jax.device_count()})"
    n_samples_per_device = len(self.r) // jax.device_count()

    def _split(x):
      x = x.reshape(
        (
          jax.process_count(), jax.local_device_count(), n_samples_per_device,
          *x.shape[1:]
        )
      )
...
def _resize_array(x, new_length):
  ...
  if new_length < old_length:
    ...
    return jnp.concatenate(
      [x for _ in range(n_replicas)] + [x[:n_remainder, ...]], axis=0
    )
...
@functools.partial(
  jax.vmap, in_axes=(MCMC_BATCH_AXES,), out_axes=(MCMC_BATCH_AXES, 0)
)
...
def _calculate_stepsize_and_langevin_bias(
  r, R, Z, stepsize, mcmc_langevin_scale, mcmc_r_min, mcmc_r_max
):
...
def _propose_step_local_stepsize(
  state: MCMCState, config: LocalStepsizeProposalConfig
):
  new_state = copy.copy(state)
  new_state.rng_state, subkey = jax.random.split(state.rng_state)

  dist_closest = jnp.min(
    get_el_ion_distance_matrix(state.r, state.R)[1], axis=-1
  )
  ...
  dist_closest = jnp.min(
    get_el_ion_distance_matrix(new_state.r, state.R)[1], axis=-1
  )
...
def _propose_step_local_stepsize_one_el(
  state: MCMCState, config: LocalStepsizeProposalConfig
):
  new_state = copy.copy(state)
  new_state.rng_state, subkey = jax.random.split(state.rng_state)
  ...
  s = state.stepsize * jnp.clip(dist_closest, config.r_min, config.r_max)
  new_state.r = new_state.r.at[..., index, :].add(
    jax.random.normal(subkey, state.r.shape[:-2] + (3,)) * s[..., None]
  )
  ...
  dist_sqr = jnp.sum(
    (new_state.r.at[..., index, :].get() - state.r.at[..., index, :].get())**2,
    axis=-1
  )
  log_q_ratio = 3 * (jnp.log(s) - jnp.log(s_new))
  log_q_ratio += 0.5 * dist_sqr * (
    1 / s**2 - 1 / s_new**2
  )  #log_q_ratio = jnp.sum(log_q_ratio, axis=-1) # sum over electrons
  return new_state, log_q_ratio
...
def _propose_step_local_approximated_langevin(
  state: MCMCState, config: MCMCLangevinProposalConfig
):
  """
  ...
  s, g_r = _calculate_stepsize_and_langevin_bias(
    state.r, state.R, state.Z, state.stepsize, config.langevin_scale,
  ...
  s_new, g_r_new = _calculate_stepsize_and_langevin_bias(
    new_state.r, state.R, state.Z, state.stepsize, config.langevin_scale,
    config.r_min, config.r_max
  )
...
class MetropolisHastingsMonteCarlo:
  ...
  def _build_proposal_function(self):
    if self.config.proposal.name == "normal":
      ...
      self.propose = functools.partial(
        _propose_step_local_approximated_langevin, config=self.config.proposal
      )
    elif self.config.proposal.name == 'local':
      self.propose = functools.partial(
        _propose_step_local_stepsize, config=self.config.proposal
      )
      ...
      self.propose = functools.partial(
        _propose_step_local_stepsize_one_el, config=self.config.proposal
      )
    else:
  ...
  def make_mcmc_step(self, func, state: MCMCState):
    ...
    do_accept = jnp.logical_or(
      do_accept, state_new.walker_age >= self.config.max_age
    )
    state_new.walker_age = jnp.where(do_accept, 0, state_new.walker_age + 1)
    state_new.log_psi_sqr = jnp.where(
      do_accept, state_new.log_psi_sqr, state.log_psi_sqr
    )
    state_new.r = jnp.where(
      do_accept[..., np.newaxis, np.newaxis], state_new.r, state.r
    )
    ...
    state_new.stepsize = jax.lax.cond(
      ...
      self._adjust_stepsize, lambda x: x[0], (state.stepsize, state.acc_rate)
    )
    return state_new
  ...
  def _adjust_stepsize(self, args):
    ...
    stepsize = jax.lax.cond(
      acceptance_rate < self.config.target_acceptance_rate, lambda s: s / 1.05,
      lambda s: s * 1.05, stepsize
    )
    stepsize = jnp.clip(
      stepsize, self.config.min_stepsize_scale, self.config.max_stepsize_scale
    )
    return stepsize
  ...
  def run_inter_steps(self, func, state: MCMCState, params, fixed_params):
    return self._run_mcmc_steps(
      func, state, params, fixed_params, self.config.n_inter_steps
    )

  @functools.partial(pmap, static_broadcasted_argnums=(0, 1))
  def run_burn_in(self, func, state: MCMCState, params, fixed_params):
    return self._run_mcmc_steps(
      func, state, params, fixed_params, self.config.n_burn_in
    )
