...
def eval_gaussian_orbital(
  el_ion_diff, el_ion_dist, ind_nuc, alpha, weights, angular_momenta
):
  """
...
def eval_atomic_orbitals(
  el_ion_diff, el_ion_dist, orbitals, ao_cusp_params=None
):
  """
    Args:
...
def evaluate_molecular_orbitals(
  ...
  aos = eval_atomic_orbitals(
    el_ion_diff, el_ion_dist, atomic_orbitals, ao_cusp_params
  )
  mos = aos @ mo_coeff
  ...
  if mo_cusp_params is not None:
    ...
    sto = _eval_cusp_molecular_orbital(el_ion_dist, r_c, offset, sign,
                                       poly) - jnp.sum(
                                         aos[..., np.newaxis, :, np.newaxis] *
                                         coeff_1s,
                                         axis=-2
                                       )
    sto = jnp.sum(
      ...
      for ind_contraction in range(weights.shape[1]):
        ...
        for m in range(
          ...
          shape_string = ao_labels[ind_basis][
            3]  # string of the form 'xxy' or 'zz'
          ...
          atomic_orbitals.append(
            [
              np.array(ind_nuc, int), alpha,
              weights[:, ind_contraction] * normalization, angular_momenta
            ]
          )
          ind_basis += 1
  assert len(
    atomic_orbitals
  ) == n_basis, "Could not properly construct basis functions. You probably tried to use a valence-only basis (e.g. cc-pVDZ) for an all-electron calculation."  # noqa
  return atomic_orbitals


def build_pyscf_molecule_from_physical_config(
  physical_config: PhysicalConfig, basis_set
):
...
def get_baseline_solution(
  physical_config: PhysicalConfig, casscf_config: CASSCFConfig, n_dets: int
):
  n_el, n_up, R, Z = physical_config.get_basic_params()
  n_dn = n_el - n_up
  atomic_orbitals, hf = get_hartree_fock_solution(
    physical_config, casscf_config.basis_set
  )
  ...
  if n_dets == 1 or casscf_config.only_hf:
    ...
    casscf = pyscf.mcscf.UCASSCF(
      hf, physical_config.n_cas_orbitals, physical_config.n_cas_electrons
    )
    casscf.kernel()
    ...
    if n_dets < len(ci_weights):
      ...
      share_captured = np.sum(ci_weights[ind_largest]**2
                             ) / np.sum(ci_weights**2)
      logger.debug(
        f"Share of CI-weights captured by {n_dets} dets: {share_captured:.3%}"
      )
  ...
  if casscf_config.cusps and (casscf_config.cusps.cusp_type == "mo"):
    mo_cusp_params = [
      calculate_molecular_orbital_cusp_params(
        atomic_orbitals, mo_coeff[i], R, Z,
        casscf_config.cusps.r_cusp_el_ion_scale
      ) for i in range(2)
    ]
  elif casscf_config.cusps and (casscf_config.cusps.cusp_type == 'ao'):
    ao_cusp_params = [
      _calculate_ao_cusp_params(*ao, physical_config.Z[ao[0]])
      for ao in atomic_orbitals
    ]

  return (
    atomic_orbitals, ao_cusp_params, mo_cusp_params, mo_coeff, ind_orbitals,
    ci_weights
  ), (E_hf, E_casscf)


def _get_effective_charge(
  Z: int, n: int, s_lower_shell=0.85, s_same_shell=0.35
):
  """
  ...
  for n_shell in range(1, n + 1):
    ...
    if n_shell == n:
      ...
      n_electrons_in_shell = min(
        Z - n_el_in_lower_shells, n_electrons_in_shell
      ) - 1
...
def _get_atomic_orbital_envelope_exponents(
  physical_config: PhysicalConfig, basis_set
):
  molecule = build_pyscf_molecule_from_physical_config(
    physical_config, basis_set
  ...
  for ind_nuc in range(n_ions):
    ...
    assert (nr_of_s_orbitals + nr_of_p_orbitals + nr_of_d_orbitals
           ) == len(ao_nuc), "Initialization currently does support d-orbitals"
...
def get_envelope_exponents_from_atomic_orbitals(
  physical_config: PhysicalConfig, basis_set='6-31G', pad_full_det=False
):
  """
  ...
  (atomic_orbitals, _, _, mo_coeff, _, _), _ = get_baseline_solution(
    physical_config, CASSCFConfig(basis_set=basis_set, cusps=None), n_dets=1
  )
  mo_coeff = mo_coeff[0][:, :n_up], mo_coeff[1][:, :n_dn
  ...
  for spin in range(2):
    mo_weights = mo_coeff[spin]**2 / np.sum(
      mo_coeff[spin]**2, axis=0
    ...
    for ind_mo in range([n_up, n_dn][spin]):
      for ind_nuc in range(n_ions):
        ...
        alpha_values[spin][
          ind_nuc,
          ind_mo] = (np.dot(mo_weights_nuc, alpha_ao[index_nuc]) + 1e-6) / (
  ...
  if pad_full_det:
    coeff_values[0] = np.concatenate(
      [coeff_values[0], np.ones([n_ions, n_dn])], axis=1
    )
    alpha_values[0] = np.concatenate(
      [alpha_values[0], np.ones([n_ions, n_dn])], axis=1
    )
    coeff_values[1] = np.concatenate(
      [np.ones([n_ions, n_up]), coeff_values[1]], axis=1
    )
    alpha_values[1] = np.concatenate(
      [np.ones([n_ions, n_up]), alpha_values[1]], axis=1
    )
...
def split_results_into_spins(hf):
  if len(hf.mo_occ.shape) == 2:
    ...
    return [hf.mo_coeff,
            hf.mo_coeff], [hf.mo_energy,
                           hf.mo_energy], [hf.mo_occ / 2, hf.mo_occ / 2]
...
def _get_orbitals_by_cas_type(casscf):
  ...
  active_orbitals = [
    list(range(n_core[s], n_core[s] + n_cas[s])) for s in range(2)
  ]
...
##################################### Cusp functions #####################################
##########################################################################################
def _get_local_energy_of_cusp_orbital(
...
def _calculate_mo_cusp_params(
  ...
  poly = np.array(
    [
      x5, x4, 6 * x1 / rc2 - 3 * x2 / r_c + 0.5 * x3 - 3 * x4 / r_c -
      6 * x5 / rc2 - 0.5 * x2 * x2, -8 * x1 / rc3 + 5 * x2 / rc2 - x3 / r_c +
      3 * x4 / rc2 + 8 * x5 / rc3 + x2 * x2 / r_c, 3 * x1 / rc4 - 2 * x2 / rc3 +
      0.5 * x3 / rc2 - x4 / rc3 - 3 * x5 / rc4 - 0.5 * x2 * x2 / rc2
    ]
  )
...
def _calculate_ao_cusp_params(ind_nuc, alpha, gto_coeffs, angular_momenta, Z):
  ...
  A = jnp.array(
    [
      [r_c**2, r_c**3, r_c**4], [2 * r_c, 3 * r_c**2, 4 * r_c**3],
      [2, 6 * r_c, 12 * r_c**2]
    ]
  )
  b = jnp.array(
    [
      jnp.log(phi_shifted * sign) - p0 - p1 * r_c, phi_rc_1 / phi_shifted - p1,
...
def calculate_molecular_orbital_cusp_params(
  ...
  n_molecular_orbitals, n_nuclei, n_atomic_orbitals = mo_coeff.shape[1], len(
    R
  ), len(atomic_orbitals)
  ...
  for nuc_idx in range(n_nuclei):
    for mo_idx in range(n_molecular_orbitals):
      ...
      for i, (_, alpha, weights, _) in enumerate(atomic_orbitals):
        if is_centered_1s[i]:
          phi_rc_0 += np.sum(weights * np.exp(-alpha * r_c**2)
                            ) * mo_coeff[i, mo_idx]
          phi_rc_1 += np.sum(
            weights * (-2 * alpha * r_c) * np.exp(-alpha * r_c**2)
          ) * mo_coeff[i, mo_idx]
          phi_rc_2 += np.sum(
            weights * (-2 * alpha + 4 *
                       (alpha * r_c)**2) * np.exp(-alpha * r_c**2)
          ) * mo_coeff[i, mo_idx]
      cusp_1s_coeffs[
        nuc_idx, :, mo_idx
      ] = is_centered_1s * mo_coeff[:, mo_idx
                                   ]  # n_nuc x n_atomic_orbitals x n_molec_orbitals
      cusp_offset[nuc_idx, mo_idx], cusp_sign[nuc_idx, mo_idx], cusp_poly[
        nuc_idx, mo_idx] = _calculate_mo_cusp_params(
