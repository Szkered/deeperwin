...
def build_local_rotation_matrices(phys_config: PhysicalConfig, tol=1e-6):
  ...
  center_of_mass = np.sum(
    R_ions * np.array(phys_config.Z)[:, None], axis=0
  ) / np.sum(phys_config.Z)
...
def _align_2D_subspace_along_ref_axes(U_sub, U_ref, tol=1e-6):
  ...
  raise AssertionError(
    "Provided axes did not have significant overlap with any of the reference axes."
  )


def _fix_degenerate_directions(U, eigvals, U_ref=None, tol=1e-6):
  deg_subspaces = _get_degenerate_subspaces(U, eigvals, tol)
  if (len(deg_subspaces) == 3) or (
    U_ref is None
  ):  # eigenvalues are not degenerate or there is no reference to use => keep as is
    return np.concatenate([x for x in deg_subspaces], axis=-1)

  assert len(
    deg_subspaces
  ) > 1, "There should not be a fully degenerate subspace when a reference axis has already been established"
...
#%%
  ...
  fig.tight_layout()
  plt.savefig(
    "/home/mscherbela/ucloud/results/local_coords_H4_sweep.png",
    bbox_inches='tight'
  )
