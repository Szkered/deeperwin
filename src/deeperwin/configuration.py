...
class ConfigBaseclass(BaseModel):
  ...
  @classmethod
  def update_configdict_and_validate(
    cls, config_dict: dict, config_changes: Iterable[Tuple[str, Any]]
  ):
    ...
    for key, value in config_changes.items():
      set_with_flattened_key(
        config_dict, key, get_with_flattened_key(parsed_config, key)
      )
...
class EmbeddingConfigFermiNet(ConfigBaseclass):
  ...
  def _set_n_iterations(cls, n, values):
    if n is None:
      try:
        ...
        raise ValueError(
          f"n_hidden_one_el must be a list of net-widths if n_iterations is not specified"
        )
    return n
  ...
  def _set_n_hidden(cls, values):
    ...
    if len(values['n_hidden_one_el']) != (len(values['n_hidden_two_el']) + 1):
      raise ValueError(
        f"Number of layers for 1-el-stream must be one more than nr of layers in 2-el-stream"
      )
    return values
...
EmbeddingConfigType = Union[EmbeddingConfigDeepErwin4, EmbeddingConfigFermiNet,
...
class MCMCConfig(ConfigBaseclass):
  ...
  proposal: Union[MCMCSimpleProposalConfig, LocalStepsizeProposalConfig,
                  MCMCLangevinProposalConfig] = MCMCSimpleProposalConfig()
...
class _OptimizerConfigAdamForPretraining(StandardOptimizerConfig):
  ...
  lr_schedule: Union[ConstantLRSchedule,
                     InverseLRScheduleConfig] = ConstantLRSchedule()
...
class _OptimizerConfigAdamForKFAC(StandardOptimizerConfig):
  ...
  learning_rate = 2e-3
  lr_schedule: Union[
    _InverseLRScheduleConfigForKFACwithAdam, InverseLRScheduleConfig,
    ConstantLRSchedule] = _InverseLRScheduleConfigForKFACwithAdam()
...
class OptimizerConfigKFAC(ConfigBaseclass):
  ...
  internal_optimizer: Union[_OptimizerConfigSGD,
                            StandardOptimizerConfig] = _OptimizerConfigSGD()
...
class _OptimizerConfigKFACwithAdam(OptimizerConfigKFAC):
  ...
  lr_schedule: Union[
    _InverseLRScheduleConfigForKFACwithAdam, InverseLRScheduleConfig,
    ConstantLRSchedule] = _InverseLRScheduleConfigForKFACwithAdam()
  internal_optimizer: Union[
    _OptimizerConfigAdamForKFAC,
    StandardOptimizerConfig] = _OptimizerConfigAdamForKFAC()
...
class OptimizationConfig(ConfigBaseclass):
  ...
  optimizer: Union[OptimizerConfigKFAC, _OptimizerConfigKFACwithAdam,
...
class PhysicalConfig(ConfigBaseclass):
  ...
  def _generate_el_ion_mapping(R, Z, n_el, n_up):
    ...
    for n in range(n_el):
      ...
      spin_to_assign = np.concatenate(
        [np.zeros(n_ions, dtype=int),
         np.ones(n_ions, dtype=int)]
      )
      ion_to_assign = np.concatenate(
        [np.arange(n_ions, dtype=int),
         np.arange(n_ions, dtype=int)]
      )
  ...
  _DEFAULT_MOLECULES = ruamel.yaml.YAML().load(
    pathlib.Path(__file__).parent.joinpath('molecules.yaml')
  )
  ...
  def populate_n_up(cls, v, values):
    if v is None:
      if values.get('name') in cls._PERIODIC_TABLE:
        ...
        spin = 0
      return (
        values['n_electrons'] + spin + 1
      ) // 2  # if there is an extra electrons, assign them to up
    else:
      return v
  ...
  def populate_el_ion_mapping(cls, v, values):
    if v is None:
      if ('name' in cls._DEFAULT_MOLECULES
         ) and ('el_ion_mapping' in cls._DEFAULT_MOLECULES[values['name']]):
        v = cls._DEFAULT_MOLECULES[values['name']]['el_ion_mapping']
      else:
        v = cls._generate_el_ion_mapping(
          values['R'], values['Z'], values['n_electrons'], values['n_up']
        )
  ...
  def set_from_changes(self):
    ...
    for change in self.changes:
      p = PhysicalConfig.update_configdict_and_validate(
        self.dict(), change.as_flattened_dict()
      )[1]
      p.changes = None
      ret.append(p)
    return ret
...
class BasicLoggerConfig(LoggerBaseConfig):
  ...
  log_level: Union[int, Literal["CRITICAL", "FATAL", "ERROR", "WARN", "WARNING",
                                "INFO", "DEBUG"]] = "DEBUG"
  n_skip_epochs: int = 9
...
class DispatchConfig(ConfigBaseclass):
  system: Literal["local", "local_background", "vsc3", "vsc4", "dgx",
                  "auto"] = "auto"
  ...
  queue: Literal["default", "vsc3plus_0064", "devel_0128", "gpu_a40dual",
                 "gpu_gtx1080amd", "gpu_gtx1080multi", "gpu_gtx1080single",
                 "gpu_v100", "gpu_k20m", "gpu_rtx2080ti", "jupyter",
                 "normal_binf", "vsc3plus_0256", "mem_0096", "devel_0096",
                 "jupyter", "mem_0096", "mem_0384", "mem_0768",
                 "gpu_a100_dual"] = "default"
...
class PreTrainingConfig(ConfigBaseclass):
  ...
  optimizer: Union[
    _OptimizerConfigAdamForPretraining,
    StandardOptimizerConfig] = _OptimizerConfigAdamForPretraining()
...
class Configuration(ConfigBaseclass):
  ...
  model: Union[ModelConfigDeepErwin4, ModelConfigFermiNet,
               ...
               ModelConfigJCB2e, ModelConfigNC,
  ...
  def no_reuse_while_shared(cls, values):
    if (values['optimization'].shared_optimization
        is not None) and (values['reuse'] is not None):
      raise ValueError(
        "Re-using weights not currently supported for shared optimization"
      )
    return values
  ...
  for key, value in nested_dict.items():
    if isinstance(value, dict):
      flattened_dict.update(
        {key + "." + k: v for k, v in build_flattend_dict(value).items()}
      )
    else:
...
if __name__ == '__main__':
  import pathlib
  c = Configuration.parse_obj(
    dict(physical=dict(name='LiH'), model=dict(name="dpe4"))
  )
  p = pathlib.Path(__file__).parent.joinpath(
    "../../sample_configs/config_schema.json"
  ).resolve()
  print("Updating: ", p)
  with open(p, 'w') as f:
