...
def main():
  ...
  parser_run = subparsers.add_parser(
    "run",
    help="Run a single DeepErwin calculation with a given config.yml file"
  )
  parser_run.add_argument(
    "config_file", default="config.yml", help="Path to input config file"
  )

  # Sub-parser for weight-sharing run
  parser_run_shared = subparsers.add_parser(
    "run-shared",
  ...
  parser_setup.add_argument(
    "--input", "-i", default="config.yml", help="Path to input config file"
  )
  parser_setup.add_argument(
    '--parameter', '-p', nargs='+', action='append', default=[]
  )
  parser_setup.add_argument(
    '--force',
    '-f',
  ...
  parser_available_gpus = subparsers.add_parser(
    "select-gpus",
    help="Setup and dispatch one or multiple DeepErwin calculations, e.g. for a parameter sweep"
  )
  parser_available_gpus.add_argument(
    "--n-gpus", default=1, type=int, help="Number of GPUs required"
  )
  parser_available_gpus.add_argument(
    "--sleep",
    default=0,
    type=int,
    help="Sleep for given number of seconds to avoid collisions before querying available gpus"
  )

  # Sub-parser for Weights&Biases sweep-agent
  subparsers.add_parser(
    "wandb-agent",
    help="Agent to be called by wandb sweep-agent for automatic hyperparameter search"
  )
