...
def get_ion_ion_potential_energy(R, Z):
  ...
  E_pot = jnp.triu(
    charge_matrix / (dist_matrix + eye), k=1
  )  # add eye to diagonal to prevent div/0
  return jnp.sum(E_pot, axis=[-2, -1])
...
def _calculate_ion_ion_forces(R, Z):
  ...
  forces = diff * Z_matrix[..., jnp.newaxis] / (
    dist + EPSILON
  )**3  # tiny EPS ensures that the diagonal (self-interaction) has 0 contribution
...
def calculate_forces(
  ...
  if config.use_antithetic_sampling:
    ...
    diff_to_closest_ion = jax.vmap(jax.vmap(lambda x, ind: x[ind]))(
      diff_el_ion, ind_closest_ion
    )  # vmap over number of electrons and batch-size
    is_core_electron = d_closest_ion < config.R_core

    r_mirrored = jnp.where(is_core_electron, r - 2 * diff_to_closest_ion, r)
    diff_el_ion_mirrored, r_el_ion_mirrored = get_el_ion_distance_matrix(
      r_mirrored, R
    )
    mirrored_weight = jnp.exp(
      log_sqr_func(params, r_mirrored, R, Z, fixed_params) - log_psi_sqr
    )

    forces_mirrored = _calculate_forces_directly(
      diff_el_ion_mirrored, r_el_ion_mirrored, Z, config.R_cut
    )
    forces = (
      forces + forces_mirrored * mirrored_weight[:, None, None, None]
    ) * 0.5

  force = jnp.mean(
    jnp.sum(forces, axis=1), axis=0
  )  # sum over electrons, average over batch
